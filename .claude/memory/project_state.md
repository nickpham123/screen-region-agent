...
- **Step 5 — Chat Panel (next unstarted item; Step 4 is now closed, see Built and working)** — merged question+answer interface, multi-turn, text + voice input (voice via `whisper-node-addon`/whisper.cpp, on-device, held with the same key used for region selection)
